...
task Align {
    ...
    command <<<
        ...
        dorado aligner "~{draft_asm}" "~{reads}" | samtools sort --threads "$NPROC" > "~{sample_id}"_aligned_dorado.bam
        echo "Finished aligning reads to draft assembly! Now indexing bam."
        samtools index "~{sample_id}"_aligned_dorado.bam
        echo "Finished indexing!"
    >>>

    output {
        File bam = "~{sample_id}_aligned_dorado.bam"
        File bai = "~{sample_id}_aligned_dorado.bam.bai"
    }
